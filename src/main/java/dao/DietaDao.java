...
public class DietaDao extends GenericDao<Dieta> {
    ...
    public List<Object[]> relatorioDietasMaisPopulares() {
        EntityManager em = getEntityManager();
        try {
            String relatorioDieta = "SELECT d.descricao, COUNT(r) FROM Receita r " + "JOIN r.dieta d " + "GROUP BY d.descricao " +  "ORDER BY COUNT(r) DESC";
            
            return em.createQuery(relatorioDieta, Object[].class)
                     .getResultList();
        } finally {
            em.close();
        }
